...
The full likelihood of an observed graph $y$ and order $s$ is then just the product of these logistic likelihoods
$$
p(y|s,\theta) = \prod_{t=1}^{n_d}p(y_{s_t}=1 | \eta, y^{t-1}, s_{ \leq t}),
$$
and the marginal likelihood of an observed graph is just the sum over all possible orderings
$$
...
Key terms still to be added are:
...
All user facing functions in the `lolog` package accept `BinaryNet`s as arguments, and will convert `network` objects to `BinaryNet`s automatically.
